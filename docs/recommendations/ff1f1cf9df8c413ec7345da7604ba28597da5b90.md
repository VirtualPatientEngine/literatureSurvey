...
<body>
  <p>
  <i class="footer">This page was last updated on 2026-09-21 06:39:06 UTC</i>
  </p>
  ...
  <tbody>
        ...
        <tr id="This work studies the problem of time series analysis with generalist (or foundation) models, which are models trained across many data domains. Drawing inspiration from the widespread success of large language models, we consider the simple strategy of discretely tokenizing time series data drawn from a myriad of datasets via self-supervision, then using the fixed tokenization to solve a variety of tasks across many data domains. Canonically, time series models are either trained on a single dataset or built in a task-specific manner (e.g., a forecasting-only model), where many use patches of time as inputs to the model. As such, performant generalist, discrete representation time series models explored across many tasks are of value. Our method, TOkenized Time Series EMbeddings (TOTEM), produces such generalist time series models with minimal or no fine-tuning while exhibiting strong zero-shot performance. We evaluate TOTEM extensively over nearly 500 experiments on three commonly-studied time series tasks with real-world data: imputation (17 baselines, 12 datasets), anomaly detection (19 baselines, 25 datasets), and forecasting (14 baselines, 12 datasets). We conclude that TOTEM matches or outperforms existing state-of-the-art models in both the canonical specialist setting (i.e., training one model on one domain) as well as the generalist setting (i.e., training a single model on many domains), which demonstrates the efficacy of tokenization for general time series analysis. The open-source implementation is available here: https://github.com/SaberaTalukder/TOTEM; a video summary is available here: https://www.youtube.com/watch?v=OqrCpdb6MJk.">
          ...
          <td>
            Sabera J. Talukder, Yisong Yue, Georgia Gkioxari
          </td>
          <td>2024-02-26</td>
        ...
        <tr id="Generative pre-training has recently become a promising route for building transferable time series models, while language-model reprogramming has shown that task descriptions and statistical context can also be useful for temporal prediction. These two ideas, however, are usually studied in separate settings. Native time series Transformers are trained directly on numerical sequences, but they are rarely conditioned on explicit task or dataset information. LLM-based methods can use prompts, yet their backbones are mainly pre-trained on text rather than temporal signals. In this paper, we propose PromptTimer, a prompt-guided generative Transformer for time series analysis. The model preserves the autoregressive structure of Timer and introduces a lightweight reprogramming layer inspired by Time-LLM. Specifically, time series are represented as segment tokens, while task, horizon, and statistical descriptors are encoded as prompt tokens. The reprogramming layer aligns these two token spaces before decoding. Forecasting, imputation, and residual-based anomaly detection are then formulated under the same conditional generation framework. Experiments on six benchmarks indicate that prompt-guided reprogramming can improve the official Timer backbone for segment reconstruction, although residual-based anomaly detection remains sensitive to score calibration.">
          <td id="tag"><i class="material-icons">visibility_off</i></td>
          <td><a href="https://www.semanticscholar.org/paper/fcc29c4c5cebeb9f789c016e35757d4abfa06f41" target='_blank'>PromptTimer: Prompt-Guided Generative Transformer for Time Series Analysis</a></td>
          <td>
            Yixin Liu, Peihong Yang, Yu Luo, Xiangyu Song
          </td>
          <td>2026-05-01</td>
          <td>2026 IEEE International Conference on Pattern Recognition, Machine Vision and Artificial Intelligence (PRMVAI)</td>
          <td>0</td>
          <td>8</td>
        </tr>
    
        <tr id="Despite recent progress in time-series foundation models, challenges persist in improving representation learning and adapting to diverse downstream tasks. We introduce a General Time-series Model (GTM), which advances representation learning via a novel frequency-domain attention mechanism that captures time-granularity-aware features, an aspect underexplored in prior research. We further propose a novel pre-training strategy that unifies reconstruction and autoregressive objectives through a hybrid masking mechanism. Our pre-training strategy, combined with 2D positional encoding and span shuffling, enhances the robustness and generalization of representations. GTM is established as the first generative-task-agnostic model for time-series analysis, enabling seamless adaptation to various generative tasks without any task-specific modifications. Extensive experiments demonstrate that GTM consistently outperforms SOTA models on various generative tasks and achieves strong classification results with minimal adaptation. Furthermore, GTM exhibits clear scaling behavior, with accuracy improving as model size and pre-training data increase.">
          <td id="tag"><i class="material-icons">visibility_off</i></td>
          <td><a href="https://www.semanticscholar.org/paper/4071bb82eb1ab1828ec4fc1678f41be581e417cd" target='_blank'>GTM: A General Time-series Model for Enhanced Representation Learning of Time-Series Data</a></td>
          <td>
